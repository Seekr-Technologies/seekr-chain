...
class UserConfig:
    ...
    nix_store_max_size :
        Size budget for the hostPath store, parsed via ``utils.human_to_int``
        (``"50GiB"`` = 1024-based, ``"100G"`` = 1000-based, bare ``"1024"`` =
        bytes). When chain-nix-init finishes a pull, it checks store size; if above this limit, it
