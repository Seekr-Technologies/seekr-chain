...
def _build_controller_jobset(
    ...
    Runs the controller as a JobSet (rather than a plain batch/v1 Job) so that
    every self-patch/self-annotate the controller does on its own resource is a
    jobset.x-k8s.io/jobsets RBAC call — the same permission workers already
    need, and one the Argo Workflows fallback ServiceAccount actually grants
    (unlike batch/jobs, which it doesn't).
    """
