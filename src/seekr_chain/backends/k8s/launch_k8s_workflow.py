...
from seekr_chain.backends.k8s.jobset import _INIT_IMAGE, build_jobset_context, create_jobset_manifest
...
_DEFAULT_CONTROLLER_IMAGE = "ghcr.io/seekr-technologies/seekr-chain-controller:1.0.0"
...
def _create_workflow_secrets(config: WorkflowConfig, workflow_name: str, s3_creds: dict) -> list[dict]:
    ...
    for cred_key in (s3_creds or {}).keys():
        out.append({"name": cred_key.upper(), "valueFrom": {"secretKeyRef": {"name": workflow_name, "key": cred_key.upper()}}})
