...
``controller_status_watcher()`` watches only the controller JobSet and its
phases ConfigMap to back ``K8sWorkflow.watch_controller_status()`` (used by
``wait()``, which may be watching many jobs concurrently and only needs the
overall status).
...
class WatchDisconnection:
    ...
    kind: str  # "job" | "phases" | "jobsets" | "pods"
    attempt: int
    max_attempts: int
...
class WatchSpec:
    ...
    kind: str  # "job" | "phases" | "jobsets" | "pods" — also the cache key and failure-tracking key
...
def _seed_phases_configmap(k8s_v1, namespace, workflow_id) -> tuple[list, str]:
    """Seed the phases-ConfigMap cache from a single read — same 0-or-1-element
    pattern as ``_seed_controller_jobset()``, for the same reason."""
    cm = read_phases_configmap(k8s_v1, namespace, workflow_id)
    if cm is None:
        return [], ""
    return [cm], cm.metadata.resource_version or ""


def _phases_configmap_spec(k8s_v1, namespace, workflow_id) -> WatchSpec:
    return WatchSpec(
        kind="phases",
        seed=functools.partial(_seed_phases_configmap, k8s_v1, namespace, workflow_id),
        list_fn=k8s_v1.list_namespaced_config_map,
        list_kwargs=dict(
            namespace=namespace,
            field_selector=f"metadata.name={workflow_id}-phases",
        ),
        key=_v1_name,
        rv=_v1_rv,
    )
...
def workflow_state_watcher(
    ...
    specs = [
        ...
        _phases_configmap_spec(k8s_v1, namespace, workflow_id),
        WatchSpec(
            kind="jobsets",
...
def controller_status_watcher(
    ...
    not a full per-step/pod ``WorkflowState`` — two watches here instead of
    four (no fanned-out worker JobSets/Pods) keeps that cheap.
    """
    return ReconnectingWatcher(
        label=workflow_id,
        specs=[
