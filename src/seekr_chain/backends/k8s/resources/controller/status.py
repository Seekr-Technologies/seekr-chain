...
# it and uploads it via s5cmd, so it need not live on the shared workspace.
# A single stable path keeps the async shipper's coalesce-by-re-reading-latest.
_STATUS_PATH = os.path.join(tempfile.gettempdir(), "seekr-chain-status.json")
...
def _shipper_loop() -> None:
    ...
    A single long-lived thread + Event (rather than one upload per write) is
    what makes this coalescing work: signaling while an upload is already in
    flight just leaves the event set, so the loop ships once more with
    whatever is latest on disk after the current upload finishes — at least
    one upload of the latest content, never a queue of stale ones."""
    while True:
        _ship_event.wait()
        _ship_event.clear()
        _ship_once()
