...
from seekr_chain.utils import format_bytes, human_to_int, resolve_image
...
def _resolve_nix_role(role_config, code_path: str | None = None) -> dict:
    ...
    # Resolve store first — cheapest check, fails fastest.
    store_uri = _resolve_store_uri(nix, role_config.name or "")

    # Repeat eval is cheap: nix's internal eval store caches it from resolve_nix_steps.
    closure = _eval_role_closure(nix, code_path)
    closure_hash = nix_utils.closure_hash_from_path(closure)

    # build=False fast-fails here instead of waiting for the pod's nix copy --from to 404.
    ...
    return {
        ...
        "init_env": [
            ...
            {"name": "SEEKR_CHAIN_NIX_CLOSURE", "value": closure},
            {
                ...
                "value": str(
                    human_to_int(_user_config.nix_store_max_size)
...
def _detect_warm_nodes(role_config) -> list[str]:
    ...
    roles via :func:`nix_resolution.find_warm_nodes`. Build steps (image-mode
...
def _merge_affinity_with_closure(
    ...
    2. **nodeAffinity** on ``kubernetes.io/hostname In [warm_nodes]`` (weight 90).
       ...
       time by :func:`nix_resolution.find_warm_nodes`). nodeAffinity matches
       against node labels directly so it works regardless of pod liveness.

    Both are soft — under capacity pressure pods spread to cold nodes rather
    than blocking. Weights compound when a node matches both terms.
...
def _compute_role_affinity(
    role_config, code_path: str | None, workflow_affinity: dict | None
) -> tuple[str | None, dict | None]:
    """Return ``(closure_hash, role_affinity)`` for a role's rendered pod.

    Wraps closure-hash + warm-node detection and affinity merging into a
    single call; see :func:`_detect_closure_hash` and
    :func:`_merge_affinity_with_closure` for the detailed contracts.
    """
    ...
    return closure_hash, role_affinity
...
def _select_role_runtime(role_config, *, code_path: str | None, interactive: bool):
    ...
    1. **Nix-mode consumer** (``role.nix`` set). Main container runs the
       ...
       with ``subPath=nix`` — the SAME mount chain-nix-init itself uses
       (see ``_nix_init_container.yaml.j2``), so init and main see an
       identical, unified store: writes land on disk at
       ``/nix/store/<hash>`` (relative to the hostPath's ``nix/``
       subdirectory) and surface in main at the same
       ``/nix/store/<hash>`` — the path that the closure's RPATHs were
       baked against. chain-nix-init bootstraps this mount from the
       runner image's own preserved ``/nix-baked`` before running any
       real nix command (replacing an earlier chroot-store design —
    ...
    2. **Auto-injected builder** (image-mode + ``SEEKR_CHAIN_NIX_CLOSURE``
       ...
       ``nix build`` + ``nix copy --to s3``. Same mount shape as case 1 —
       ``/nix`` with ``subPath=nix`` — so ``nix build`` writes land
       directly on the node's shared hostPath as each derivation is
       realized; no separate copy-out step needed. There's no init
       container here (the build script IS main), so ``nix-build.sh``
       invokes the same bootstrap-from-``/nix-baked`` step directly,
       rather than via a dedicated init container. A consumer pod
       scheduled to the same node by closure-hash podAffinity will find
       the closure already on disk and its ``nix copy --from`` becomes a
       no-op.
...
def _build_role_context(
    ...
    closure_hash, role_affinity = _compute_role_affinity(role_config, code_path, workflow_affinity)
