    ...
    if container_statuses:
        for container_status in container_statuses:
            ...
            if container_status.state.waiting:
                ...
                # The Kubernetes API guarantees V1ContainerState always has exactly one
                # of waiting/running/terminated set (protobuf oneof). This branch should
                # never be reached, but we degrade gracefully rather than crashing
                # `chain status` if the client library ever changes its deserialization.
                logger.warning(
                    "Unexpected container state for %r: not waiting, running, or terminated — treating as UNKNOWN",
                    container_status.name,
                )
                container_state.status = ContainerStatus.UNKNOWN
...
def _is_jobset_suspended(k8s_custom, name: str, namespace: str) -> bool:
