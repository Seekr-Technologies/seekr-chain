...
2. **`chain-nix-init`** (init container, runs after chain-init):
    - Mounts the node's shared hostPath (or emptyDir) volume directly at
      `/nix` (`subPath=nix`), which shadows the runner image's own `/nix`.
      Before anything else, `nix-bootstrap.sh` reseeds `/nix` from
      `/nix-baked` — a full copy of the image's own toolchain, preserved
      at image-build time specifically for this — so a working `nix`
      exists on this now-empty-or-partially-warm mount. Content-addressed
      paths are immutable, so this reseed is a cheap no-op once a node's
      `/nix` already has the toolchain from an earlier pod.
    - If the closure root + all transitive deps are already at
      `/nix/store/<hash>` (warm cache from a previous pod on this node),
      it exits in ~1 s without touching s3.
...
The build pod (see below) uses the identical bootstrap-then-mount
approach — it's just `nix-build.sh` invoking `nix-bootstrap.sh` directly
rather than a separate init container doing it.
...
- **`/nix-baked`**: a full copy of the image's own `/nix`, preserved at
  build time (`RUN cp -a /nix /nix-baked`). Both `chain-nix-init.sh` and
  `nix-build.sh` mount the shared hostPath/emptyDir volume directly at
  `/nix`, shadowing this image's own copy — `/nix-baked` is what
  reseeds that mount before either script runs a real nix command. Adds
  roughly one toolchain's worth of image size; an accepted cost of the
  bootstrap design, not an oversight.
...
Nothing else beyond the preserved toolchain snapshot. Everything your
job actually needs comes from the closure.
...
│   nix-bootstrap.sh: seed /nix from          │
│     /nix-baked (once per node)              │
│   cd /seekr-chain/workspace                 │
│   nix build path:./#...                     │  ← lands on shared /nix
│                                              │     as each path builds
...
Both the build pod and the consumer pod mount the same node-shared `/nix`
directly — there's no separate chroot path for either anymore. A build
pod and a consumer pod that land on the same fresh node coordinate their
`/nix-baked` reseed via the same lock file, so only one of them actually
does the copy.
...
Every pod that touches nix — consumer role, `chain-nix-init` init
container, and auto-injected build step alike — mounts it identically:

| Mount point | subPath | Why |
|---|---|---|
| `/nix` | `nix` | Surfaces the hostPath's `nix/` subdirectory at `/nix` in the container — exactly where the closure's binaries' RPATHs expect `/nix/store/<hash>` to be. Without `subPath`, a container would see `/nix/nix/store/<hash>` instead. |

Mounting the shared volume at `/nix` shadows whatever the runner image
itself had there, so every one of these containers runs
`nix-bootstrap.sh` first (`chain-nix-init.sh` and `nix-build.sh` invoke
it directly; there's no separate init container for the build step). It
reseeds `/nix` from `/nix-baked` — the image's own preserved copy — so a
working `nix` toolchain exists before any real nix command runs. See
`resources/nix-bootstrap.sh`'s header comment for the full mechanics
(atomic-lock coordination between pods on the same node, dead-lock
reclaim, etc.).

This replaced an earlier design where the auto-injected build step
mounted a separate `/nix-shared` path and used `--store
local?root=/nix-shared` to redirect its reads/writes there while
leaving the image's own `/nix` untouched for the actual build. That
approach worked but carried real complexity: a separate substituter
entry for local warm-cache reads, an explicit `nix copy --to` after
every successful build to seed the shared volume, and an `EXIT` trap to
repeat that seed on failure (since a failed build's progress never
reached the happy-path copy). Bootstrapping `/nix` directly — the same
approach the consumer side already used — eliminates all of that: `nix
build` writes each derivation straight into the shared store as it's
realized, so there's nothing left to copy out or rescue on failure.
...
1. Reads on-disk size of `/nix` (or uses the value chain-nix-init
...
3. Otherwise: writes a symlink to the just-pulled closure under
   `/nix/var/nix/gcroots/seekr-chain/active` (so nix's GC
