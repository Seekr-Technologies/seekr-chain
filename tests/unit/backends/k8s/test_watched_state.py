...
fetch_fn, fakes the Kubernetes Watch API: k8s_api.new_watch() is patched so
...
def make_fake_watch_class(events_by_kind):
    """Return a fake replacement for the Watch class k8s_api.new_watch builds.
...
def test_seed_produces_first_snapshot_without_waiting_for_watch_events(monkeypatch):
    ...
    monkeypatch.setattr(watched_state_mod.k8s_api, "new_watch", fake_watch_cls)
...
def test_continuous_watch_failures_escalate_to_watch_stalled_error(monkeypatch):
    ...
    monkeypatch.setattr(watched_state_mod.k8s_api, "new_watch", _FailingWatch)
