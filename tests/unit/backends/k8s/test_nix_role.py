...
class TestNixRendering:
    def test_chain_nix_init_container_present(self, tmp_path, monkeypatch):
        ...
        # Mounts the shared volume directly at /nix with subPath=nix —
        # the same mount main uses — replacing the image's own /nix
        # entirely. chain-nix-init.sh bootstraps it from the image's
        # preserved /nix-baked before running any real nix command.
        ...
        assert mounts["nix-store"] == "/nix"
        assert mounts["workspace"] == "/seekr-chain"
        nix_store_mount = next(m for m in nix_init["volumeMounts"] if m["name"] == "nix-store")
        assert nix_store_mount["subPath"] == "nix"
    ...
    def test_size_parser_handles_iec_suffixes(self):
        ...
        assert human_to_int("50GiB") == 50 * 1024**3
        assert human_to_int("100M") == 100 * 1000**2
        assert human_to_int("1024") == 1024
        assert human_to_int("1TiB") == 1024**4
    ...
    def test_main_container_mounts_nix_volume(self, tmp_path, monkeypatch):
        ...
        # subPath: nix is mandatory because the hostPath volume's root
        # has a `nix/` subdirectory containing the actual store (both
        # chain-nix-init and main mount it this same way). Without
        # subPath, main would see /nix/nix/store/<hash> and miss.
    ...
    def test_build_step_pod_gets_closure_label(self, tmp_path, monkeypatch):
        ...
        cfg = resolve_nix_steps(cfg, staged_code_dir=tmp_path, staging_dir=tmp_path)
        ...
        # Build pod mounts the hostPath volume at /nix with subPath=nix —
        # the SAME shape a consumer pod uses. nix-build.sh bootstraps this
        # mount from /nix-baked itself (no separate init container) and
        # `nix build` writes land directly on the shared store as each
        # derivation is realized, so a consumer pod scheduled to the same
        # node via closure-hash podAffinity finds the closure already
        # present and its `nix copy --from` is a no-op.
        main = next(c for c in pod["spec"]["containers"] if c["name"] == "main")
        nix_mount = next(m for m in main["volumeMounts"] if m["name"] == "nix-store")
        assert nix_mount["mountPath"] == "/nix"
        assert nix_mount["subPath"] == "nix"
        ...
        # rather than consuming one; its bootstrap happens inline in main.
        init_names = [c["name"] for c in pod["spec"]["initContainers"]]
