...
class TestJobsetTemplateRendering:
    ...
    def test_single_replicated_job(self, tmp_path):
        ...
        # Single-role steps use role name "" (an empty replicatedJob name),
        # which YAML round-trips as null.
        assert jobs[0]["name"] is None
    ...
    def test_single_role_master_addr_matches_replicated_job_name(self, tmp_path):
        ...
        # Single-role name is "" (rendered empty, which YAML parses back as null).
        replicated_job_name = manifest["spec"]["replicatedJobs"][0]["name"] or ""
