...
class TestExamples:
    ...
    def test_renders(self, dir_path: Path, tmp_path, monkeypatch):
        ...
        if _config_uses_nix(config):
            ...
            # jobset.py and nix_resolution.py each hold their own
            # `_user_config` binding (both `from seekr_chain.user_config
            # import config as _user_config` at import time), so both must
            # be patched -- patching only jobset's leaves
            # nix_resolution._resolve_store_uri() reading the real
            # process-wide singleton.
            fake_user_config = UserConfig(
                nix_store="s3://fake-test-bucket",
                nix_runner_image="registry.example.com/nix-runner:test",
            )
            monkeypatch.setattr(jobset_mod, "_user_config", fake_user_config)
            monkeypatch.setattr(nr_mod, "_user_config", fake_user_config)
            monkeypatch.setattr(
