...
# nix system strings match `uname -m` on linux (x86_64, aarch64). macOS's
# platform.machine() reports Darwin's arch names (e.g. "arm64"), not Linux's
# ("aarch64"), so a submit-machine running on Apple Silicon needs the
# translation below to land on a system the flake actually publishes.
_NIX_ARCH = {"arm64": "aarch64"}.get(platform.machine(), platform.machine())
_NIX_SYSTEM = f"{_NIX_ARCH}-linux"
...
class TestNixMode:
    def test_build_then_warm(self, nix_basic_dir, s3_client, minio_service):
        ...
        # nix's own S3 client (used by nix-build.sh's `nix copy`, running
